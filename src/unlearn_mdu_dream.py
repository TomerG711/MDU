"""
MDU unlearning on TOFU with Dream-7B-Instruct.

Mirrors the LLaDA implementation; the only differences are
(1) trainer base class is DreamTrainer to use Dream's right_shift_logits path,
(2) cart geo-p loss weighting default, (3) Dream model defaults.

Run:
    cd ./dllm
    CUDA_VISIBLE_DEVICES=0 accelerate launch \
        src/unlearn_mdu_dream.py \
        --model_name_or_path ./checkpoints/dream-tofu-sft/checkpoint-final \
        --tofu_path ./TOFU/forget10.json \
        --retain_path ./TOFU/retain_perturbed.json \
        --output_dir ./outputs/mdu-dream-tofu \
        --loss_type null_anchor --match_mode random --novel_percentile 100 \
        --null_anchor_eta 0.0 --null_anchor_kl_dir forward \
        --alpha 1.0 --num_train_epochs 5 --learning_rate 1e-5
"""
...
class MDUTrainer(DreamTrainer):
    ...
    def _null_anchor_kl(self, logits_c, logits_u):
        ...
        log_pc = F.log_softmax(logits_c.float(), dim=-1)
        log_pt = F.log_softmax(target_logits.float(), dim=-1).detach()
        if self.null_anchor_kl_dir == "reverse":
            pt = log_pt.exp()
            return (pt * (log_pt - log_pc)).sum(dim=-1)
        if self.null_anchor_kl_dir == "js":
            pc = log_pc.exp(); pt = log_pt.exp()
            pm = 0.5 * (pc + pt)
            log_pm = pm.clamp_min(1e-12).log()
            kl1 = (pc * (log_pc - log_pm)).sum(dim=-1)
            kl2 = (pt * (log_pt - log_pm)).sum(dim=-1)
            return 0.5 * (kl1 + kl2)
        # default: forward KL(p_c ‖ p_target)
        pc = log_pc.exp()
        return (pc * (log_pc - log_pt)).sum(dim=-1)
    ...
    def _random_sft_null_anchor_loss(self, model, input_ids, labels, attention_mask):
        ...
        # uncond forward (frozen): also mask Q positions
...
class ModelArguments(dllm.utils.ModelArguments):
    model_name_or_path: str = "./checkpoints/dream-tofu-sft/checkpoint-final"
...
@dataclass
class TrainingArguments(DreamTrainer.DreamConfig):
    output_dir: str = "./outputs/mdu-dream-tofu"
    ...
    # Dream-specific defaults inherited from DreamConfig:
    #   loss_weight_type = "cart[geo_p:0.3]"
...
def train():
    ...
    logger.info(
        ...
        f"forget={len(forget_ds)}, retain={len(retain_ds)}"
    )
    ...
    import glob, shutil
    # Copy Dream model code so the saved checkpoint is loadable standalone.
    DREAM_PY_SRC = "./checkpoints/dream-tofu-sft/checkpoint-final"
    for src in glob.glob(f"{DREAM_PY_SRC}/*.py"):
        try:
